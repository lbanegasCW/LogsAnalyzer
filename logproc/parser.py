...
El parser es puro y sin estado para ser seguro en multiproceso y testeable.
"""
...
def parse_line(line: str) -> Optional[ParsedLine]:
    """Parsea una línea cruda a campos normalizados.

    Parámetros:
        line: Línea cruda del log.

    Retorna:
        Tupla ``(url, status_code, response_time_ms)`` cuando el parseo
        es exitoso; en caso contrario ``None`` para líneas malformadas.

    Complejidad:
        ``O(k)`` donde ``k`` es la longitud de la línea.
    """
